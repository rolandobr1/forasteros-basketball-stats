...
import { XMarkIcon, UsersIcon } from '../utils'; // Added UsersIcon
...
interface PlayerSelectionModalProps extends Omit<DialogProps, 'title'> {
  ...
  onGoToRosterManagement?: () => void; // New prop
}
...
const PlayerSelectionModal: React.FC<PlayerSelectionModalProps> = React.memo(({
  ...
  onGoToRosterManagement, // Destructure new prop
  ...
  const handleGoToRoster = useCallback(() => {
    if (onGoToRosterManagement) {
      onGoToRosterManagement();
    }
    onClose();
  }, [onGoToRosterManagement, onClose]);

  if (!isOpen) return null;

  const noPlayersInRoster = roster.length === 0;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-brand-surface rounded-lg shadow-xl p-6 w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div className="overflow-y-auto mb-4 flex-grow pr-2">
          {noPlayersInRoster ? (
            <div className="text-center py-4">
              <UsersIcon className="w-16 h-16 text-gray-400 dark:text-slate-500 mx-auto mb-3" />
              <p className="text-gray-600 dark:text-slate-300">No hay jugadores en la plantilla global.</p>
              <p className="text-sm text-gray-500 dark:text-slate-400">Por favor, añádelos en la gestión de jugadores para poder seleccionarlos.</p>
            </div>
          ) : (
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-slate-700">
          ...
          >
            {noPlayersInRoster ? "Cerrar" : "Cancelar"}
          </button>
          {noPlayersInRoster ? (
             onGoToRosterManagement && (
                <button
                    onClick={handleGoToRoster}
                    className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md flex items-center focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50"
                >
                    <UsersIcon className="w-5 h-5 mr-2" />
                    Ir a Gestión de Jugadores
                </button>
             )
          ) : (
            <button
              onClick={handleConfirm}
              className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50"
              disabled={currentSelection.length === 0 && !noPlayersInRoster}
            >
