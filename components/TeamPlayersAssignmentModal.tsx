...
const TeamPlayersAssignmentModal: React.FC<TeamPlayersAssignmentModalProps> = React.memo(({
  isOpen, onClose, team, allPlayers, allTeams, onAssignPlayers,
}) => {
  const [selectedPlayerIds, setSelectedPlayerIds] = useState<Set<string>>(new Set());

  useEffect(() => { if (isOpen) setSelectedPlayerIds(new Set(team.playerIds)); }, [isOpen, team.playerIds]);

  const isPlayerAssignedToAnotherTeam = useCallback((playerId: string): boolean => {
    return allTeams.some(t => t.id !== team.id && t.playerIds.includes(playerId));
  }, [allTeams, team.id]);
  ...
  const handleConfirm = useCallback(() => { onAssignPlayers(team.id, Array.from(selectedPlayerIds)); onClose(); }, [onAssignPlayers, team.id, selectedPlayerIds, onClose]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-brand-surface rounded-lg shadow-xl p-6 w-full max-w-md max-h-[80vh] flex flex-col">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-white">Asignar Jugadores a "{team.name}"</h2>
          ...
          {allPlayers.length === 0 ? (
            ...
            <ul className="space-y-2">
              {allPlayers.map(player => {
                ...
                const otherTeamName = assignedToOtherTeam ? allTeams.find(t => t.id !== team.id && t.playerIds.includes(player.id))?.name : null;
                return (
                  <li key={player.id} 
                    ...
                    <input type="checkbox" id={`player-${player.id}`} checked={selectedPlayerIds.has(player.id)}
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-slate-600">
          ...
          <button onClick={handleConfirm} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50">
