...
const PlayerStatsModal: React.FC<PlayerStatsModalProps> = React.memo(({
  ...
  const handleStatChange = useCallback((statType: StatType, increment: boolean) => {
    setStats(currentLocalStats => {
        ...
        const newStatValue = Math.max(0, newValue); // Ensure stat doesn't go below 0

        let updatedStats = { ...currentLocalStats, [statType]: newStatValue };

        // Handle dependent stats (e.g., MADE implies ATTEMPTED)
        if (increment) { // Only when incrementing a "MADE" shot
            if (statType === StatType.POINTS_1_MADE) {
                updatedStats[StatType.POINTS_1_ATTEMPTED] = (updatedStats[StatType.POINTS_1_ATTEMPTED] || 0) + 1;
            } else if (statType === StatType.POINTS_2_MADE) {
                updatedStats[StatType.POINTS_2_ATTEMPTED] = (updatedStats[StatType.POINTS_2_ATTEMPTED] || 0) + 1;
            } else if (statType === StatType.POINTS_3_MADE) {
                updatedStats[StatType.POINTS_3_ATTEMPTED] = (updatedStats[StatType.POINTS_3_ATTEMPTED] || 0) + 1;
            }
        } else { // When decrementing, ensure consistency for ATTEMPTED shots affecting MADE shots
            if (statType === StatType.POINTS_1_ATTEMPTED) {
                if (newStatValue < (updatedStats[StatType.POINTS_1_MADE] || 0)) {
                    updatedStats[StatType.POINTS_1_MADE] = newStatValue;
                }
            } else if (statType === StatType.POINTS_2_ATTEMPTED) {
                if (newStatValue < (updatedStats[StatType.POINTS_2_MADE] || 0)) {
                    updatedStats[StatType.POINTS_2_MADE] = newStatValue;
                }
            } else if (statType === StatType.POINTS_3_ATTEMPTED) {
                if (newStatValue < (updatedStats[StatType.POINTS_3_MADE] || 0)) {
                    updatedStats[StatType.POINTS_3_MADE] = newStatValue;
                }
            }
        }

        // Foul out alert logic
        if (allowFoulOuts && statType === StatType.FOULS_PERSONAL && increment && newStatValue === maxPersonalFouls) {
        ...
        onSaveStats(player, teamType, updatedStats); // Autosave with the calculated new stats
        return updatedStats; // Update local state
    });
  }, [allowFoulOuts, maxPersonalFouls, player, teamType, onSaveStats, setAlertInfo]);


  const handleCloseAlert = useCallback(() => setAlertInfo({ ...alertInfo, isOpen: false }), [alertInfo]);
  
  // Moved early return to after all hook calls
    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center p-4 z-50">
      <div className="bg-white dark:bg-brand-surface rounded-lg shadow-xl p-6 w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-slate-700">
          <button onClick={onClose} className="px-4 py-2 bg-brand-accent text-white rounded-md hover:bg-opacity-90">Cerrar</button>
        </div>
      </div>
    </div>
