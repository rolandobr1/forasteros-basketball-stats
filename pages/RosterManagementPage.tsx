...
import { PlusIcon, EditIcon, DeleteIcon, SaveIcon, XMarkIcon, ChevronUpIcon, ChevronDownIcon, BasketballIcon, CheckCircleIcon, CircleIcon, ImportIcon, ExportIcon, AddUserIcon } from '../utils'; // Added AddUserIcon, removed UsersIcon from direct use here for this button
    ...
    return (
      <div className="bg-white dark:bg-brand-surface p-3 rounded-md shadow-lg space-y-2">
        ...
        <div className="flex space-x-2 justify-end">
          <button onClick={onSaveEdit} className="p-1.5 bg-green-600 hover:bg-green-700 text-white rounded-md"><SaveIcon className="w-4 h-4" /></button>
          <button onClick={onCancelEdit} className="p-1.5 text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-white bg-gray-200 dark:bg-slate-600 rounded-md"><XMarkIcon className="w-4 h-4" /></button>
        </div>
      </div>
    );
  }
...
const RosterManagementPageComponent: React.FC<RosterManagementPageProps> = ({
  ...
  const handleConfirmCreateTeamWithPlayers = useCallback((selectedPlayerIdsForTeam: string[]) => { 
    onAddTeam(pendingTeamNameForCreation, selectedPlayerIdsForTeam); setNewTeamName(''); setIsCreateTeamModalOpen(false); setPendingTeamNameForCreation('');
  ...
  const handleExportPlayersCSV = useCallback(() => {
    if (players.length === 0) {
      setAlertInfo({isOpen: true, title: "Sin Jugadores", message: "No hay jugadores para exportar."});
      return;
    }
    const headers = "ID_Original,Nombre,Numero,Posicion\n";
    const csvRows = players.map(p => 
      [p.id, p.name, p.number, p.position || ''].map(field => `"${String(field).replace(/"/g, '""')}"`).join(',')
    ).join('\n');
    const csvContent = "data:text/csv;charset=utf-8," + headers + csvRows;
    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `forasteros_jugadores_export_${new Date().toISOString().slice(0,10)}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  }, [players]);

  const handlePlayerImportCSV = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      ...
      if (lines.length <= 1) {
        setAlertInfo({isOpen: true, title: "Archivo Vacío o Inválido", message: "El archivo CSV está vacío o no contiene datos de jugadores."});
        return;
      }
      ...
      const positionIndex = headers.indexOf('posicion');

      if (nameIndex === -1 || numberIndex === -1) {
        setAlertInfo({isOpen: true, title: "Cabeceras Faltantes", message: "El archivo CSV debe contener las cabeceras 'Nombre' y 'Numero'."});
        return;
      }

      const importedPlayersData: Array<Omit<Player, 'id' | 'position'> & { position?: string }> = [];
      let skippedRows = 0;
      for (let i = 1; i < lines.length; i++) {
        ...
        if (name && number) {
          importedPlayersData.push({ name, number, position });
        } else {
          skippedRows++;
        }
      }
      if (importedPlayersData.length > 0) onAddPlayersBatch(importedPlayersData);
      setAlertInfo({isOpen: true, title: "Importación Completada", message: `${importedPlayersData.length} jugadores importados. ${skippedRows > 0 ? `${skippedRows} filas omitidas por datos faltantes.` : ''}`});
      if(playerImportRef.current) playerImportRef.current.value = ""; // Reset file input
    };
    reader.readAsText(file);
  }, [onAddPlayersBatch]);

  const handleExportSingleTeamJSON = useCallback((teamToExport: Team) => {
    if (!teamToExport) return;
    const teamPlayers = teamToExport.playerIds
        .map(playerId => players.find(p => p.id === playerId))
        .filter(player => player !== undefined) as Player[];

    const exportData = {
        id: teamToExport.id,
        name: teamToExport.name,
        players: teamPlayers, // Embed full player objects
    };
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = `forasteros_equipo_${teamToExport.name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    setAlertInfo({isOpen: true, title: "Exportación Exitosa", message: `Equipo "${teamToExport.name}" exportado.`});
  }, [players]);

  const handleExportAllTeamsJSON = useCallback(() => { // Renamed for clarity
    if (teams.length === 0) {
      setAlertInfo({isOpen: true, title: "Sin Equipos", message: "No hay equipos para exportar."});
      return;
    }
    // This exports teams with only playerIds, not full player objects.
    const jsonString = JSON.stringify(teams, null, 2);
    const blob = new Blob([jsonString], { type: "application/json" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `forasteros_todos_los_equipos_${new Date().toISOString().slice(0,10)}.json`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
    setAlertInfo({isOpen: true, title: "Exportación Exitosa", message: "Todos los equipos han sido exportados."});
  }, [teams]);


  const handleTeamImportJSON = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!fileContent) {
          setAlertInfo({isOpen: true, title: "Error de Importación", message: "El archivo está vacío."});
          return;
        }
        const parsedData = JSON.parse(fileContent);

        if (Array.isArray(parsedData)) { // Handles array of teams (old format or bulk export)
          let importedCount = 0;
          let skippedCount = 0;
          (parsedData as Partial<Team>[]).forEach(importedTeam => {
            if (importedTeam.name && Array.isArray(importedTeam.playerIds)) {
              const validPlayerIds = importedTeam.playerIds.filter(id => typeof id === 'string' && players.some(p => p.id === id));
              onAddTeam(importedTeam.name, validPlayerIds as string[]); // onAddTeam creates new ID, checks for name collision
              importedCount++;
            } else {
              skippedCount++;
            }
          });
          setAlertInfo({isOpen: true, title: "Importación de Equipos Completada", message: `${importedCount} equipos importados desde array. ${skippedCount > 0 ? `${skippedCount} equipos omitidos por formato incorrecto.` : ''}`});
        
        } else if (typeof parsedData === 'object' && parsedData !== null && parsedData.id && parsedData.name && Array.isArray(parsedData.players)) {
          // Handles single team object with embedded players (new format)
          const singleTeam = parsedData as { id: string; name: string; players: Player[] };
          
          if (singleTeam.players.some(p => !p.id || !p.name || typeof p.number === 'undefined')) {
             setAlertInfo({isOpen: true, title: "Error de Importación", message: "Formato de jugador inválido en el archivo de equipo."});
             return;
          }
          
          upsertPlayersToRoster(singleTeam.players);
          upsertTeam({ 
            id: singleTeam.id, 
            name: singleTeam.name, 
            playerIds: singleTeam.players.map(p => p.id) 
          });
          setAlertInfo({isOpen: true, title: "Importación de Equipo Exitosa", message: `Equipo "${singleTeam.name}" y ${singleTeam.players.length} jugador(es) importados/actualizados.`});
        } else {
          setAlertInfo({isOpen: true, title: "Error de Importación", message: "Formato de archivo JSON no reconocido."});
        }
      } catch (error) {
        setAlertInfo({isOpen: true, title: "Error de Importación", message: "El archivo JSON es inválido o no tiene el formato esperado."});
    ...
    reader.onerror = () => {
        setAlertInfo({isOpen: true, title: "Error de Lectura", message: "No se pudo leer el archivo."});
        if(teamImportRef.current) teamImportRef.current.value = "";
    };
    reader.readAsText(file);
  }, [onAddTeam, players, upsertPlayersToRoster, upsertTeam]);
    ...
    <div className="container mx-auto px-4">
      <div className="space-y-6">
        ...
        <div className="bg-gray-50 dark:bg-brand-dark p-4 rounded-lg shadow-xl space-y-4">
          ...
          <div className="bg-white dark:bg-brand-surface p-4 rounded-lg shadow-md space-y-3">
            ...
            <div className="flex space-x-2">
              <button onClick={handleAddNewPlayer} className="flex-1 px-3 py-1.5 text-sm bg-green-600 text-white rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 flex items-center justify-center"><SaveIcon className="w-4 h-4 mr-1"/>Añadir Jugador</button>
           ...
           <div className="bg-white dark:bg-brand-surface p-3 rounded-md shadow space-y-2 md:space-y-0 md:flex md:items-center md:justify-between">
            <h3 className="text-md font-medium text-gray-700 dark:text-white">Importar/Exportar Jugadores</h3>
            <div className="flex space-x-2">
              <input type="file" ref={playerImportRef} onChange={handlePlayerImportCSV} accept=".csv" className="hidden" />
              <button onClick={() => playerImportRef.current?.click()} className="px-3 py-1.5 text-xs bg-sky-600 text-white rounded-md hover:bg-sky-700 flex items-center"><ImportIcon className="w-4 h-4 mr-1"/>Importar Jugadores (CSV)</button>
              <button onClick={handleExportPlayersCSV} className="px-3 py-1.5 text-xs bg-sky-700 text-white rounded-md hover:bg-sky-800 flex items-center"><ExportIcon className="w-4 h-4 mr-1"/>Exportar Jugadores (CSV)</button>
            </div>
          </div>
        ...
        <div className="bg-gray-50 dark:bg-brand-dark p-4 rounded-lg shadow-xl space-y-4">
          ...
          <div className="bg-white dark:bg-brand-surface p-4 rounded-lg shadow-md space-y-3">
            ...
            <input type="text" placeholder="Nombre del Nuevo Equipo" value={newTeamName} onChange={(e) => setNewTeamName(e.target.value)} 
              className="w-full p-2 rounded bg-gray-100 dark:bg-slate-700 text-gray-800 dark:text-white border border-gray-300 dark:border-slate-600 focus:border-brand-accent select-auto"/>
            <button onClick={handleOpenCreateTeamModal} className="w-full px-4 py-1.5 text-sm bg-green-600 text-white rounded-md hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-50 flex items-center justify-center"><PlusIcon className="w-4 h-4 mr-1"/>Crear Equipo y Asignar Jugadores</button>
          </div>
          {/* Team Import/Export Section */}
          <div className="bg-white dark:bg-brand-surface p-3 rounded-md shadow space-y-2 md:space-y-0 md:flex md:items-center md:justify-between">
            <h3 className="text-md font-medium text-gray-700 dark:text-white">Importar/Exportar Lista de Equipos</h3>
            <div className="flex space-x-2">
              <input type="file" ref={teamImportRef} onChange={handleTeamImportJSON} accept=".json" className="hidden" />
              <button onClick={() => teamImportRef.current?.click()} className="px-3 py-1.5 text-xs bg-sky-600 text-white rounded-md hover:bg-sky-700 flex items-center"><ImportIcon className="w-4 h-4 mr-1"/>Importar Equipos (JSON)</button>
              <button onClick={handleExportAllTeamsJSON} className="px-3 py-1.5 text-xs bg-sky-700 text-white rounded-md hover:bg-sky-800 flex items-center"><ExportIcon className="w-4 h-4 mr-1"/>Exportar Todos (JSON)</button>
            </div>
          </div>
                ...
                <div key={team.id} className="bg-white dark:bg-brand-surface p-3 rounded-md shadow">
                  {editingTeamId === team.id ? (
                    <div className="flex items-center space-x-2">
                      ...
                      <button onClick={handleSaveTeamName} className="p-1.5 bg-green-600 hover:bg-green-700 text-white rounded-md"><SaveIcon className="w-5 h-5" /></button>
                      <button onClick={() => setEditingTeamId(null)} className="p-1.5 text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-white bg-gray-200 dark:bg-slate-600 rounded-md"><XMarkIcon className="w-5 h-5" /></button>
                    </div>
                  ) : (
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        <button onClick={() => handleStartEditTeamName(team)} className="p-1.5 text-blue-500 hover:text-blue-400 dark:text-blue-400 dark:hover:text-blue-300" aria-label="Editar Nombre"><EditIcon className="w-5 h-5" /></button>
                        <button onClick={() => handleExportSingleTeamJSON(team)} className="p-1.5 text-teal-500 hover:text-teal-400 dark:text-teal-400 dark:hover:text-teal-300" aria-label={`Exportar ${team.name} (JSON)`}><ExportIcon className="w-5 h-5"/></button>
                        <button onClick={() => handleOpenAssignModal(team)} className="p-1.5 text-purple-500 hover:text-purple-400 dark:text-purple-400 dark:hover:text-purple-300" aria-label="Gestionar Jugadores"><AddUserIcon className="w-5 h-5"/></button>
                        <button onClick={() => handleDeleteTeamRequest(team.id)} className="p-1.5 text-red-500 hover:text-red-400 dark:text-red-400 dark:hover:text-red-300" aria-label="Eliminar Equipo"><DeleteIcon className="w-5 h-5" /></button>
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div className="mt-8">
          <Link
            to="/setup"
            className="w-full flex items-center justify-center px-6 py-2.5 text-sm bg-green-600 text-white rounded-md hover:bg-green-700 font-semibold shadow-lg focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-opacity-75"
          >
...
const RosterManagementPage = React.memo(RosterManagementPageComponent);
